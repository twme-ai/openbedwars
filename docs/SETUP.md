...
Buying or picking up a stone, iron, or diamond sword removes the default wooden sword instead of occupying a second slot. Purchased swords are ordinary match items: players can drop them, move them into containers, and retrieve them later. When the last upgraded sword leaves the player inventory and cursor, exactly one wooden fallback is restored on the next server tick. Purchased swords are still lost on death, and the respawn kit restores the same fallback.

The default wooden sword, shears, pickaxes, and axes cannot be moved out of the player inventory during a match. OpenBedWars blocks shift transfers, cursor placement, number-key and offhand swaps, drags into a top inventory, and bundle insertion; cursor and slot drop actions remain blocked as well. Players can still rearrange this permanent equipment inside their own inventory. Stone, iron, and diamond swords are deliberately excluded from this restriction.
...
Every arena-tracked entity also carries an OpenBedWars marker. A persistent Dream Defender that unloads during its current match keeps its UUID ownership and resumes normal targeting when the chunk loads again. Arena reset clears that ownership; if a marked entity from the ended match is loaded later, OpenBedWars removes it on the next server tick after Paper completes the chunk-load transition. Plugin startup also removes marked leftovers that are already loaded.
